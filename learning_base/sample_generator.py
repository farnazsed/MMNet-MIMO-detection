...
import numpy as np
from utils import *
import numpy.linalg as LA  # Import numpy.linalg and alias it as LA
...
class generator(object):
    ...
    def QAM_N_ind(self):
        ...
        if self.mod_scheme == 'MIX':
            ...
            # mod_names: 0-->QAM64, 1-->QAM16, 2-->QAM4
    ...
    def channel(self, x, snr_db_min, snr_db_max, H, dataset_flag, correlation_flag):
        ...
        if dataset_flag:
            ...
            Hr = tf.random.normal(shape=[self.batch_size, self.NR, self.NT], stddev=1. / np.sqrt(2. * self.NR),
                                  dtype=tf.float32)
            Hi = tf.random.normal(shape=[self.batch_size, self.NR, self.NT], stddev=1. / np.sqrt(2. * self.NR),
                                  dtype=tf.float32)
        ...
        # Channel Noise
        snr_db = tf.random.uniform(shape=[self.batch_size, 1], minval=snr_db_min, maxval=snr_db_max, dtype=tf.float32)
        if self.mod_scheme == 'MIX':
            print(self.constellation)
            powQAM4 = 10. * tf.math.log(
                (tf.square(self.constellation[0]) + tf.square(self.constellation[7])) / 2.) / tf.math.log(10.)
            powQAM16 = 10. * tf.math.log((self.constellation[0] ** 2 + self.constellation[2] ** 2 + self.constellation[
                5] ** 2 + self.constellation[7] ** 2) / 4.) / tf.math.log(10.)
        ...
        average_x_powerdB = 10. * tf.math.log(tf.reduce_mean(tf.reduce_sum(tf.square(x), axis=1))) / tf.math.log(10.)
        w *= tf.pow(10., (10. * np.log10(self.NT) + self.Hdataset_powerdB - snr_db - 10. * np.log10(self.NR)) / 20.)
        complexnoise_sigma = tf.pow(10., (
                    10. * np.log10(self.NT) + self.Hdataset_powerdB - snr_db - 10. * np.log10(self.NR)) / 20.)

        # Channel Output
        y = batch_matvec_mul(H, x) + w
        sig_powdB = 10. * tf.math.log(
            tf.reduce_mean(tf.reduce_sum(tf.square(batch_matvec_mul(H, x)), axis=1))) / tf.math.log(10.)
        noise_powdB = 10. * tf.math.log(tf.reduce_mean(tf.reduce_sum(tf.square(w), axis=1))) / tf.math.log(10.)
