...
class denoiser(object):
    ...
    def MMNet(self, zt, xhatt, rt, features, linear_helper):        
        ...
        v2_t = tf.divide((tf.reduce_sum(tf.square(rt), axis=1, keepdims=True) - self.NR * tf.square(noise_sigma) / 2.), tf.expand_dims(tf.linalg.trace(HTH), axis=1))

        v2_t = tf.maximum(v2_t , 1e-9)
        v2_t = tf.expand_dims(v2_t, axis=2)
        C_t = tf.eye(self.NT, batch_shape=[tf.shape(H)[0]]) - tf.matmul(W_t, H)
        tau2_t = 1./self.NT * tf.reshape(tf.linalg.trace(tf.matmul(C_t, C_t, transpose_b=True)), [-1,1,1]) * v2_t + tf.square(tf.reshape(noise_sigma,[-1,1,1])) / (2.*self.NT) * tf.reshape(tf.linalg.trace(tf.matmul(W_t, W_t, transpose_b=True)), [-1,1,1])
        shatt1, _ = self.gaussian(zt, {'tau2_t':tau2_t/tf.Variable(tf.random.normal([1,self.NT,1], 1., 0.1))})
        denoiser_helper = {'onsager':tf.Variable(0., trainable=False)}
        return shatt1, denoiser_helper, shatt1
    ...
    def OAMPNet(self, zt, xhatt, rt, features, linear_helper):        
        ...
        W_t = linear_helper['W']
        HTH = tf.matmul(H, H, transpose_a=True)
        v2_t = tf.divide((tf.reduce_sum(tf.square(rt), axis=1, keep_dims=True) - self.NR * tf.square(noise_sigma) / 2.), tf.expand_dims(tf.linalg.trace(HTH), axis=1))
        v2_t = tf.maximum(v2_t , 1e-9)
        v2_t = tf.expand_dims(v2_t, axis=2)
        theta_t = tf.Variable(1., trainable=True)
        C_t = tf.eye(self.NT, batch_shape=[tf.shape(H)[0]]) - theta_t * tf.matmul(W_t, H)
        tau2_t = 1./self.NT * tf.reshape(tf.linalg.trace(tf.matmul(C_t, C_t, transpose_b=True)), [-1,1,1]) * v2_t + tf.square(tf.reshape(theta_t,[-1,1,1])*tf.reshape(noise_sigma,[-1,1,1])) / (2.*self.NT) * tf.reshape(tf.trace(tf.matmul(W_t, W_t, transpose_b=True)), [-1,1,1])
    ...
    def featurous_nn(self, zt, xhatt, rt, features, linear_helper):
        ...
        feature2 = 1./tf.linalg.trace(tf.matmul(H,H, transpose_a=True))
