...
class linear(object):
    ...
    def MMNet(self, shatt, rt, features):
        ...
        Wr = tf.Variable(tf.random.normal(shape=[1, self.NT // 2, self.NR // 2], mean=0., stddev=0.01))
        Wi = tf.Variable(tf.random.normal([1, self.NT // 2, self.NR // 2], 0., 0.01))
    ...
    def MMNet_iid(self, shatt, rt, features):
        batch_size = self.batch_size
        H = features['H']
        W = tf.transpose(H, perm=[0, 2, 1])  # Use tf.transpose with the correct permutation
        helper = {'W': W}

        # Ensure proper shape alignment
        zt = shatt + tf.reshape(tf.square(tf.Variable(1.)), [-1, 1]) * batch_matvec_mul(W, rt)
    ...
    def lin_DetNet(self, shatt, rt, features):
        ...
        W = tf.transpose(H, perm=[0, 2, 1])
        helper = {'W': W}
        Theta1 = tf.Variable(tf.random.normal([1, self.NT, self.NT], 0., 0.001))
        theta2 = tf.Variable(1.)
        theta3 = tf.Variable(tf.random.normal([1, self.NT], 0., 0.001))
    ...
    def OAMPNet(self, shatt, rt, features):
        ...
        v2_t = tf.divide((tf.reduce_sum(tf.square(rt), axis=1, keepdims=True) - self.NR * tf.square(noise_sigma) / 2.),
        ...
        inv_term = tf.linalg.inv(v2_t * HHT + lam * tf.eye(self.NR, batch_shape=[tf.shape(H)[0]]))
        What_t = v2_t * tf.matmul(H, inv_term, transpose_a=True)
        W_t = self.NT * What_t / tf.reshape(tf.linalg.trace(tf.matmul(What_t, H)), [-1, 1, 1])
