...
for M in [4,16,64]:
    for s in range(snr[str(M)][0], snr[str(M)][1]+1):
        cmnd = 'python benchmark.py --x-size 16 --y-size 64 --snr-min %d --snr-max %d --batch-size 100 --numSamples 100 --modulation QAM_%d --ML --overwrite --data --data-dir D:\MMNet\data3/H_iid.npy'%(s, s, M)
